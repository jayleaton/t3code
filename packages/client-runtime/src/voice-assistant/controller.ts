 ...
 * - The local VAD owns the 5/10-second rule. Timers start at activation and
 *   reset only on accepted human speech, never on keyboard noise or assistant
 *   output.
...
export class VoiceAssistantController {
  ...
  private startCapture(includePreRoll: boolean): void {
    ...
    void this.ports.capture.beginUtterance().then(() => {
      // Finalization or cancel may have raced the async capture start; a late
      // start must not resurrect timers or upload audio for a dead session.
      if (generation !== this.generation || this.state.input !== "capturing") return;
      if (includePreRoll) {
        const preRoll = this.ports.capture.preRoll();
        if (preRoll !== null && preRoll.byteLength > 0) {
          this.ports.conversation.sendAudio(preRoll);
        }
      }
      this.restartSilenceTimer();
      // A bounded maximum window finalizes cleanly rather than streaming forever.
  ...
  async pressPushToTalk(): Promise<void> {
    ...
    // Push-to-talk always interrupts playback before capturing.
    if (this.state.output === "speaking" || this.state.output === "preparing") {
  ...
  announce(text: string): void {
    ...
    if (this.state.input === "capturing" || this.state.input === "verifying") {
